...
def simulate_gps_and_radiation(node):
    ...
    while rclpy.ok():
        ...
        node.latest_lat = lat
        node.latest_lon = lon
        node.latest_radiation = random.uniform(0, 200)  # Simulate radiation value
        node.add_point()
        time.sleep(1)
...
def main():
    ...
    def update_radiation_markers():
        ...
        for lat, lon, value in node.radiation_points:
            # Determine marker icon based on radiation value
            if value < 1:
                icon = good_icon  # Green marker
            elif 1 <= value < 20:
                icon = medium_icon  # Yellow marker
            else:
                icon = bad_icon  # Red marker

            # Set marker with custom icon and radiation value
